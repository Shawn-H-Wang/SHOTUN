...
class HOTN(nn.Module):
    def __init__(self, core_size, cube_size, threshold):
        super(HOTN, self).__init__()
        D=[]
        D.append(torch.Tensor(core_size[0], cube_size[0]).float())
        D.append(torch.Tensor(core_size[1], cube_size[1]).float())
        D.append(torch.Tensor(core_size[2], cube_size[2]).float())
        D.append(torch.Tensor(core_size[3], cube_size[3]).float())
        B=[]
    ...
    def forward(self, ct):
        ...
        """Decode"""
        clean_spatial_cubes = chain_mode_product(ct, [D4, D3, D1, D2])
        clean_spatial_cubes_3D = chain_mode_product(ct, [torch.eye(D4.shape[0]).cuda(), D3, D1, D2])
        return clean_spatial_cubes, clean_spatial_cubes_3D, D4, D3, D2, D1
...
class UNFOLDNET(DataInfo):
    ...
    def __init__(self, ndata, nratio, nsnr=0, data_iter=1, psf=None, srf=None, edm_num=80, stage=3):
        ...
        self.lr_fun = lambda epoch: (1.0 - max(0, epoch + 1 - 1400) / 600)  # decay of learning rate
        ...
        self.K = unfoldings
        threshold = 0.001
        self.block = block_module(self.cube_size[0], ws, 15, params)
        self.block_hsi = block_module(cube_size_hsi[0], ws//nratio, 15, params)
        ...
        self.l2loss = nn.MSELoss(size_average=False).cuda()# nn.MSELoss(size_average=False).cuda()
        ...
        self.hotn = HOTN(core_size=self.core_size, cube_size=self.cube_size, threshold=threshold).cuda()
        self.optimizer_hotn = optim.Adam(self.hotn.parameters(), lr=self.lr, weight_decay=self.lam_C)
        self.aggerate = AggerationModule()


        # self.model = TISTANet(self.ms_bands, self.hs_bands, self.core_size, self.cube_size, multi_lmbda, unfoldings, threshold).cuda()
    ...
    def build_loss(self, Y, Z, hsi, msi): # , basis, coeff, rec_z
        ...
        # rec_loss = self.l1loss(Y, hsi) + self.l1loss(Z, msi)
        # rec_loss = self.huber_loss(Y, hsi) + self.huber_loss(Z, msi)
        loss = rec_loss
        return loss
        ...
        while True:
            ...
            for num1 in range(num1_epochs):
                ...
                for num2 in range(num2_epochs):
                    ...
                    patches, patches_3D, D4, D3, D2, D1 = self.hotn(ct)
                    A, X_out = self.aggerate(patches, S, self.block)
                    A_3D, X_out_3D = self.aggerate(patches_3D, S, self.block)
                    Yhat, Zhat = self.cpt_target(X_out)
                    Yhat_3D, Zhat_3D = self.cpt_target(X_out_3D)
                    a1 = 1e-1
                    loss = self.build_loss(Zhat, Yhat, Z_ref, Y_ref) 
                    loss = loss + self.build_loss(Zhat_3D, Yhat_3D, Z_ref, Y_ref)
            ...
            if iteration % 20 == 0:
                ...
                Xh_3D = torchkits.to_numpy(X_out_3D)
                Xh_3D = toolkits.channel_last(Xh_3D)

                """simulated fusion experiments"""
                rmse, psnr, sam, ergas, ssim, uiqi = MetricsCal(self.ref, Xh, scale=self.nratio)
...
#             self.apply_D.append(nn.Parameter(B[-1]))
#         if multi_lmbda:
#             self.lmbda = nn.ParameterList([nn.Parameter(torch.zeros([1,core_size[3],core_size[2],core_size[0],core_size[1]])) for _ in range(unfoldings)])
