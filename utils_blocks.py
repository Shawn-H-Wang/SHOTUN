...
class block_module():
    ...
    def _agregate_blocks(self,batch_out_blocks):
        ...
        _,c,h,w = batch_out_blocks.shape
        batch_out_blocks_flatten = batch_out_blocks.reshape(-1,c*h*w).transpose(0,1).unsqueeze(0)
        ...
        if params['ponderate_out_blocks']:
            ...
            batch_out_blocks_ones = torch.ones_like(batch_out_blocks) * mask.squeeze()
            # batch_out_blocks_flatten_ones = batch_out_blocks_ones.flatten(2, 3).permute(1, 2, 0)
            batch_out_blocks_flatten_ones = batch_out_blocks_ones.reshape(-1, c*h*w).transpose(0,1).unsqueeze(0)
